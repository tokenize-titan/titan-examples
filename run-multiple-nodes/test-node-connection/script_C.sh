...
xargs -I {} docker run --rm -i -v $(pwd)/nodes/val1:/root/.titan titand:latest add-genesis-account "{}" 10000titan
# val1 stakes titan 
echo $(cat ./nodes/val1/passphrase.txt)$'\n'$(cat ./nodes/val1/passphrase.txt) | \
docker run --rm -i -v $(pwd)/nodes/val1:/root/.titan titand:latest gentx val1 1000titan --keyring-backend file --keyring-dir /root/.titan/keys --chain-id titan_90000-1 >/dev/null 2>&1
...
xargs -I {} docker run --rm -i -v $(pwd)/nodes/val2:/root/.titan titand:latest add-genesis-account "{}" 5000titan
# val2 stakes titan
echo $(cat ./nodes/val2/passphrase.txt)$'\n'$(cat ./nodes/val2/passphrase.txt) | \
docker run --rm -i -v $(pwd)/nodes/val2:/root/.titan titand:latest gentx val2 1000titan --keyring-backend file --keyring-dir /root/.titan/keys --chain-id titan_90000-1 >/dev/null 2>&1
...
xargs -I {} docker run --rm -i -v $(pwd)/nodes/val3:/root/.titan titand:latest add-genesis-account "{}" 5000titan
# val3 stakes titan
echo $(cat ./nodes/val3/passphrase.txt)$'\n'$(cat ./nodes/val3/passphrase.txt) | \
docker run --rm -i -v $(pwd)/nodes/val3:/root/.titan titand:latest gentx val3 1000titan --keyring-backend file --keyring-dir /root/.titan/keys --chain-id titan_90000-1 >/dev/null 2>&1
...
xargs -I {} docker run --rm -i -v $(pwd)/nodes/val4:/root/.titan titand:latest add-genesis-account "{}" 5000titan
# val4 stakes titan
echo $(cat ./nodes/val4/passphrase.txt)$'\n'$(cat ./nodes/val4/passphrase.txt) | \
docker run --rm -i -v $(pwd)/nodes/val4:/root/.titan titand:latest gentx val4 1000titan --keyring-backend file --keyring-dir /root/.titan/keys --chain-id titan_90000-1 >/dev/null 2>&1
...
### On val1's machine
# val1 configures to add val2, val3 as seed peers
