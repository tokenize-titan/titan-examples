...
### On val2's machine
docker run --rm -it -v $(pwd)/nodes/val2:/root/.titan titand:latest init val2 --chain-id titan_90000-1 --default-denom titan >/dev/null
# create keyring's passphrase
...
### On val3's machine
docker run --rm -it -v $(pwd)/nodes/val3:/root/.titan titand:latest init val3 --chain-id titan_90000-1 --default-denom titan >/dev/null
# create keyring's passphrase
...
### On val4's machine
docker run --rm -it -v $(pwd)/nodes/val4:/root/.titan titand:latest init val4 --chain-id titan_90000-1 --default-denom titan >/dev/null
# create keyring's passphrase
printf password > ./nodes/val4/passphrase.txt
# create account
echo $(cat ./nodes/val4/passphrase.txt)$'\n'$(cat ./nodes/val4/passphrase.txt) | \
docker run --rm -i -v $(pwd)/nodes/val4:/root/.titan titand:latest keys add val4 --keyring-backend file --keyring-dir /root/.titan/keys --output json > ./nodes/val3/val3.info

### On explorer's machine
# initialize chain
docker run --rm -it -v $(pwd)/nodes/explorer:/root/.titan titand:latest init explorer --chain-id titan_90000-1 --default-denom titan >/dev/null
...
jq '.app_state.staking.params.max_validators = 2' ./nodes/val1/config/genesis.json > ./nodes/val1/config/tmp.json && mv ./nodes/val1/config/tmp.json ./nodes/val1/config/genesis.json
...
xargs -I {} docker run --rm -i -v $(pwd)/nodes/val2:/root/.titan titand:latest add-genesis-account "{}" 10000000titan
# val2 stakes titan
echo $(cat ./nodes/val2/passphrase.txt)$'\n'$(cat ./nodes/val2/passphrase.txt) | \
docker run --rm -i -v $(pwd)/nodes/val2:/root/.titan titand:latest gentx val2 1000000titan --keyring-backend file --keyring-dir /root/.titan/keys --chain-id titan_90000-1 >/dev/null 2>&1
...
xargs -I {} docker run --rm -i -v $(pwd)/nodes/val3:/root/.titan titand:latest add-genesis-account "{}" 10000000titan
# val3 stakes titan
echo $(cat ./nodes/val3/passphrase.txt)$'\n'$(cat ./nodes/val3/passphrase.txt) | \
docker run --rm -i -v $(pwd)/nodes/val3:/root/.titan titand:latest gentx val3 1000000titan --keyring-backend file --keyring-dir /root/.titan/keys --chain-id titan_90000-1 >/dev/null 2>&1
...
xargs -I {} docker run --rm -i -v $(pwd)/nodes/val4:/root/.titan titand:latest add-genesis-account "{}" 10000000titan
# val4 stakes titan
echo $(cat ./nodes/val4/passphrase.txt)$'\n'$(cat ./nodes/val4/passphrase.txt) | \
docker run --rm -i -v $(pwd)/nodes/val4:/root/.titan titand:latest gentx val4 1000000titan --keyring-backend file --keyring-dir /root/.titan/keys --chain-id titan_90000-1 >/dev/null 2>&1
...
(docker run --rm -i -v $(pwd)/nodes/val2:/root/.titan titand:latest tendermint show-node-id | \
xargs -I{} echo {}@val2:26656; \
docker run --rm -i -v $(pwd)/nodes/explorer:/root/.titan titand:latest tendermint show-node-id | \
...
(docker run --rm -i -v $(pwd)/nodes/val1:/root/.titan titand:latest tendermint show-node-id | \
...
(docker run --rm -i -v $(pwd)/nodes/val4:/root/.titan titand:latest tendermint show-node-id | \
...
(docker run --rm -i -v $(pwd)/nodes/val3:/root/.titan titand:latest tendermint show-node-id | \
...
docker compose -f docker-compose.yml up -d val1 val2 val3 val4
