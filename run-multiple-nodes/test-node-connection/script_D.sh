...
# val1 stakes titan 
echo $(cat ./nodes/val1/passphrase.txt)$'\n'$(cat ./nodes/val1/passphrase.txt) | \
docker run --rm -i -v $(pwd)/nodes/val1:/root/.titan titand:latest gentx val1 2001000titan --keyring-backend file --keyring-dir /root/.titan/keys --chain-id titan_90000-1 >/dev/null 2>&1
...
### On explorer's machine
# explorer configures to add val1 as seed peers
...
docker compose -f docker-compose.yml up -d val1 val2 explorer
