...
# val1 stakes titan 
echo $(cat ./nodes/val1/passphrase.txt)$'\n'$(cat ./nodes/val1/passphrase.txt) | \
docker run --rm -i -v $(pwd)/nodes/val1:/root/.titand titand:latest gentx val1 1001000tkx --keyring-backend file --keyring-dir /root/.titand/keys --chain-id titan_18889-1 >/dev/null 2>&1
...
# val3 stakes titan
echo $(cat ./nodes/val3/passphrase.txt)$'\n'$(cat ./nodes/val3/passphrase.txt) | \
docker run --rm -i -v $(pwd)/nodes/val3:/root/.titand titand:latest gentx val3 500000tkx --keyring-backend file --keyring-dir /root/.titand/keys --chain-id titan_18889-1 >/dev/null 2>&1
...
# val4 stakes titan
echo $(cat ./nodes/val4/passphrase.txt)$'\n'$(cat ./nodes/val4/passphrase.txt) | \
docker run --rm -i -v $(pwd)/nodes/val4:/root/.titand titand:latest gentx val4 500000tkx --keyring-backend file --keyring-dir /root/.titand/keys --chain-id titan_18889-1 >/dev/null 2>&1
